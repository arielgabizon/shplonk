...
\begin{claim}\label{clm:div}
 ...
 $z_S(X)$ divides $f(X)-r(X)$ if and only if $Z_T$ divides $Z_{T\setminus S}\cdot (f(X)-r(X))$.
\end{claim}
...
Motivated by claim \ref{clm:div} we will make the following notational convention when describing our two PCSs. Instead of the alleged evaluations \sett{s_{i,z}}{i\in [k],z\in S_i} being an input to the \open protocol. We will give as input polynomials \sett{r_i(X)}{×}
...
\begin{claim}\label{clm:div_sum}
 ...
 Then, e.w.p $k/|\F|$ over uniform $a\in \F$, $Z$ does not divide
...
% \[]
 ...
 We know from Claim \ref{clm:div} that $F_{i^*} \defeq  Z_{T\setminus S_{i^*}}\cdot (f_{i^*}(X)-r_{i^*}(X))$ is not divisible by $Z_T$. Thus, using Claim \ref{clm:div_sum}, we know that e.w.p $k/|\F|$ over $\gamma$, $f$ is not divisble by $Z_T$.
 ...
 \begin{lemma}\label{lem:multikate}
  ...
  \begin{enumerate}
   \item For integers $n\leq d$ and $f\in \polysofdeg{n}$, computing $\com(f,\srs0)$ when $\srs=\gen(d)$ requires $n$ \G1-exponentiations.
...
Let $n,d$ be positive integers with $n\le d$, $f_i\in\polysofdeg{n}$ for $i\in[k]$, $S_i$ the opening set for $f_i$, $r_i\in\polysofdeg{|S_i|}$ such that $r_i|_{S_i} = f_i|_{S_i}$, $T= S_1\cup\ldots\cup S_k$, and $t=|T|$. There is a PCS $\PCscheme=(\gen,\com,\open)$ such that
:
\begin{enumerate}
\item the commitment $\com(f_i)$ to $f_i$ requires $\deg(f_i)$ \G1-exponentiations
\item the opening $\open(d,\set{\com(f_i)},\set{S_i},\set{r_i})$ requires
    \begin{enumerate}
...
\item $\open(d,\set{\com(f_i)},\set{S_i},\set{r_i})$ is a 2-round protocol where
    \begin{enumerate}
    \item \verpc sends a random challenge $\gamma\in \F$
    \item \prvpc sends $W=\enc1{(f/Z_T)(x)}$ where
    \[f(X)=\sum_{i\in [k]} \gamma^{i-1} Z_{T\setminus S_i}(X)(f_i(X)-r_i(X))\]
    \item \verpc sends a random evaluation point $z\in \F$
    \item \prvpc sends $W'=\enc1{f'(x)/(x-z)}$ where
    \[f'(X)=\sum_{i\in [k]} \gamma^{i-1} Z_{T\setminus S_i}(z)(f_i(X)-r_i(z)) - Z_T(z)(f/Z_T)(X)\]
...
\noindent Notice that $W$ and $W'$ are well-defined since $f$ is divisible by $Z_T$ by Claim \ref{clm:div_sum} and $f'$ is divisible by $X-z$ since $f'(z)=f(z)-Z_T(z)(f/Z_T)(z)=0$.
...
We argue knowledge soundness for the above protocol. More precisely, we argue the existence of an efficient \ext such that an algebraic adversary \adv can only win the KS game w.p. \negl.  The proof begins identically to the previous one.
 ...
 Let
 \[f(X)\defeq \sum_{i\in [t]} \gamma^{i-1} \cdot Z_{T\setminus S_i}\cdot (f_i(X)-r_i(X)).\]
 We know from Claim \ref{clm:div} that $F_{i^*} \defeq  Z_{T\setminus S_{i^*}}\cdot (f_{i^*}(X)-r_{i^*}(X))$ is not divisible by $Z_T$. Thus, using Claim \ref{clm:div_sum}, we know that e.w.p $k/|\F|$ over $\gamma$, $f$ is not divisble by $Z_T$. Assume we are in this case.  Now \adv outputs $W=\enc1{H(x)}$ for some $H\in\polysofdeg{d}$, followed
