...
% \[]
 ...
 Fix a positive integer $d$.
  There is a $d$-PCD \PCscheme such that
  \begin{enumerate}
...
Let $n,d$ be positive integers with $n\le d$, $f_i\in\polysofdeg{n}$ for $i\in[k]$, $S_i$ the opening set for $f_i$, $r_i\in\polysofdeg{|S_i|}$ such that $r_i|_{S_i} = f_i|_{S_i}$, $T= S_1\cup\ldots\cup S_k$, and $t=|T|$. Then there is a multi-point multi-polynomial commitment scheme \PCscheme such that:
\begin{enumerate}
\item the commitment $\com(f_i)$ to $f_i$ requires $\deg(f_i)$ \G1-exponentiations
\item the opening $\open\left(d,\set{\com(f_i)},\set{S_i},\set{r_i}\right)$ requires
    \begin{enumerate}
...
Notice that $W$ and $W'$ are well-defined since $f$ is divisible by $Z_T$ by Claim \ref{clm:div_sum} and $f'$ is divisible by $X-z$ since $f'(z)=f(z)-Z_T(z)(f/Z_T)(z)=0$.
