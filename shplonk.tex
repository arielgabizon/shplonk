...
Polynomial commitment schemes (PCS) are an important primitive in arguably all practical succinct proving systems (see \cite{marlin,plonk,supersonic} for formalizations of their role). \footnote{While QAP based proving systems like \cite{pinocchio,groth16} are not traditionally formulated as using a PCS, they can be interpreted as using a PCS that also forces the committed polynomial to be in a certain subspace. This is related to the linear interactive proof framework of  \cite{bciop}.} They can ``force'' a prover to answer verifier queries according to a fixed polynomial of bounded degree.
...
We describe a variant of the scheme of Section \ref{sec:main} where we eliminate the verifier's \G2 operations and reduce the number of pairings to two. This comes at the cost of extra prover communication and computation, however the scheme is still gives significant savings in proof length compared to previous results, when the numer of distinct evaluation subsets $k^*$  is small.
...
\begin{lemma}\label{lem:multikateG1}
 Fix positive integer $d$.
  ...
  \begin{enumerate}
   \item For $n\leq d$ and $f\in \polysofdeg{n}$, computing $\com(f)$ requires $n$ \G1-exponentiations.
   \item Given $T \defeq (z_1,\ldots,z_t)\in \F^t, f_1,\ldots, f_k \in \polysofdeg{d}, \sett{S_i}{i\in [k]}$, denote by $k^*$ the number of distinct subsets  \set{S^*_1,\ldots,S^*_{k^*}} in \set{S_i}; and 
   let $K\defeq t-1 + \sumi {k^*}\left( t-|S^*_i|-1\right)$.
   and denote $d^* \defeq \max\sett{\deg(f_i)}{i\in [k]}$.
   Let $\cm_i = \com(f_i)$.
   Then \open(\set{cm_i},\set{f_i},T,\set{S_i\subset T},\set{s_{i,z}})  requires
   \begin{enumerate}
   \item Two  \G1 elements and $k^*$ \F elements to be passed from \prvpoly to \verpoly.
    \item At most $d^*+1$ \G1-exponentiations of \prvpoly.
    \item $k-k^*$ \G1-exponentiations, $K$ \G2-exponentiations  and $k^*+1$ pairings of \verpoly.
    \end{enumerate}
\end{enumerate}
...
\begin{enumerate}
 ...
 \begin{enumerate}
  ...
  such that $r_i\in \polysofdeg{|S_i|}$ satisfies $r_i(z) = s_{i,z}$ for each $z\in S_i$. 
 \item \prvpc computes the polynomial
 \[g(X)\defeq \sum_{i=1}^t \gamma^{i-1}\cdot Z_{T\setminus S_i}(X)\cdot (f_i(X)-r_i(X)).\]
 and using \srs0 computes and sends $W\defeq \enc1{h(x)}$.
% \[]
 ...
 \item \prvpc computes the polynomial
 \[h(X)\defeq g(X)/Z_T(X) =    \sum_{i\in [k^*]}\frac{g_i(X)}{Z_{S_i(X)}} . \]
 and using \srs0 computes and sends $W\defeq \enc1{h(x)}$.
