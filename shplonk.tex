...
\begin{abstract}
We present an enhanced version of the Kate, Zaverucha, Goldberg polynomial commitment scheme \cite{kate} where a single group element can be an openning proof for multiple polynomials at multiple points. 
We apply this to the \plonk proof system to obtain ``for free'' constraints involving multiple shifts of a witness.
\end{abstract}
...
% \[]
 ...
 Fix positive integer $d$.
  ...
  \begin{enumerate}
   ...
   let $K\defeq t+1 + \sumi{k^*} t-|S^*_i|+1$.
   and denote $d^* \defeq \max\sett{\deg(f_i)}{i\in [k]}$.
   Let $\cm_i = \com(f_i)$.
   Then \open(\set{cm_i},\set{f_i},T,\set{S_i},\set{s_{i,z}})  requires
   \begin{enumerate}
   \item A single \G1 element to be passed from \prvpoly to \verpoly.
    \item At most $d^*+1$ \G1-exponentiations of \prvpoly.
    \item $k$ \G1-exponentiations, $K$ \G2-exponentiations  and $k^*+1$ pairings of \verpoly.
    \end{enumerate}
\end{enumerate}
